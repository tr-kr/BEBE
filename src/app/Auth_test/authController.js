...
exports.accountCheckTest = async function (req, res) {
  ...
  if (account.length == 0) return res.send(baseResponse.USER_USERID_EMPTY);

  const accountCheckResponse = await userProvider.accountCheckTest(account);
...
exports.emailCheckTest = async function (req, res) {
  const { email } = req.body;
  if (email.length == 0) return res.send(baseResponse.SIGNIN_EMAIL_EMPTY);
...
exports.updateUser = async function (req, res) {
  ...
  const signUpResponse = await userService.editUser(id, nickname, password);
...
const CLIENT_SECRET =
  "4f34a94c10adfd93b336fd0265fc8157ea9421b6a35c911e2559fa0f6c9c15d1";
